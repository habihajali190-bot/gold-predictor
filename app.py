...
# ------------------------------------------------------------------------------
# 6. قسم الحسابات الحقيقية لإدارة المخاطر الحجمية
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 8. شاشات الرصد البياني المتطور لمصفوفات السيولة المدمجة أصلياً
# ------------------------------------------------------------------------------
...
df_chart = pd.DataFrame({
    'سعر الذهب المحسوب كمياً': gold_prices,
    'مستويات الـ Order Block الدعامية': ob_levels,
    'مستويات توازن الـ Fair Value Gap': fvg_levels
}, index=dates)

# استخدام الشارت المدمج عالي الكفاءة والذي لا يتطلب أي مكتبات خارجية لمنع الأخطاء
...
# ------------------------------------------------------------------------------
st.markdown("### 📊 لوحة تسجيل وإدارة الـ Backtesting والصفقات اليومية")
st.write("استخدم الجدول التفاعلي أدناه لتسجيل تطبيق استراتيجيتك اليومية ومراقبة مدى التزامك بالقوانين الرياضية للمنصة:")

c_add1, c_add2, c_add3, c_add4 = st.columns(4)
with c_add1:
    trade_date = st.date_input("تاريخ المحاولة:", datetime.now())
with c_add2:
    trade_type = st.selectbox("نوع العملية الفنية:", ["شراء مؤسسي (Buy)", "بيع مؤسسي (Sell)"])
with c_add3:
    trade_result = st.selectbox("نتيجة الصفقة الإحصائية:", ["ربح (Take Profit)", "خسارة (Stop Loss)"])
with c_add4:
    trade_profit_r = st.number_input("العائد المحقق بمقدار المخاطرة (R-Multiple):", value=2.5, step=0.5)

if st.button("حفظ الصفقة وتحديث جدول الباك تيست التراكمي 💾"):
    st.session_state.backtest_logs.append({
        "التاريخ": trade_date.strftime("%Y-%m-%d"),
        "النوع": trade_type,
        "النتيجة": trade_result,
        "العائد (R-Value)": f"+{trade_profit_r} R" if trade_result == "ربح (Take Profit)" else f"-1.0 R"
    })
    st.success("✅ تم حفظ الصفقة بنجاح وتحديث القاعدة البيانات المباشرة للجدول.")
...
if st.session_state.backtest_logs:
    ...
    dummy_data = [
        {"التاريخ": "2026-05-15", "النوع": "شراء مؤسسي (Buy)", "النتيجة": "ربح (Take Profit)", "العائد (R-Value)": "+2.5 R"},
        {"التاريخ": "2026-05-16", "النوع": "بيع مؤسسي (Sell)", "النتيجة": "خسارة (Stop Loss)", "العائد (R-Value)": "-1.0 R"}
    ]
    st.dataframe(pd.DataFrame(dummy_data), use_container_width=True)
    st.caption("💡 الجدول أعلاه يعرض صفقات توضيحية، بمجرد قيامك بإضافة صفقاتك سيتم تصفيره وعرض بياناتك المحدثة فوراً.")

st.success("💥 النظام مجهز بشكل متكامل وشامل ومستعد لاستقبال المتداولين الآن!")
